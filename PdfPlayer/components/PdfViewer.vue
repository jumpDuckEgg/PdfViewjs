<template>
    <div style="position: relative">
        <canvas style="width: 100%; display: block" ref="canvasElt"></canvas>
        <div class="annotationLayer" ref="annotationLayerElt"></div>
...
export default {
    ...
    methods: {
        resize(size) {
            ...
            let canvasElt = this.$refs.canvasElt;
            let annotationLayerElt = this.$refs.annotationLayerElt;
    ...
    mounted(){
		...
		this.$on('pageSize', (width, height)=>{
			canvasElt.style.height = canvasElt.offsetWidth * (height / width) + 'px';
		});
        this.$on('numPages', (num)=>{
            // console.log('本pdf的长度', num);
            if(num){
                this.$emit('GET-PDF-PAGES', num);
            }
        });
        this.$on('pageLoaded', (currentPage)=>{
            // console.log(`第 ${currentPage} 页加载完成`);
            this.$emit('CURRENT-PAGE-LOADED');
        });
        this.$on('error', (err)=>{
            console.log('pdf出错：', JSON.stringify(err));
        })
		this.pdf.loadDocument(this.src);
    },
    destroyed(){
